...
if (isset($_POST['vendorDetailsStatus'])) {
	...
	if (isset($fullName) && isset($mobile) && isset($address)) {
		...
		$sql = 'INSERT INTO vendor(fullName, email, mobile, address, address2) VALUES(:fullName, :email, :mobile, :address, :address2)';
		$stmt = $conn->prepare($sql);
		$stmt->execute(['fullName' => $fullName, 'email' => $email, 'mobile' => $mobile,  'address' => $address, 'address2' => $address2]);
