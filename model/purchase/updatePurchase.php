...
if (isset($_POST['purchaseDetailsPurchaseID'])) {
	...
	if (isset($purchaseDetailsItemNumber) && isset($purchaseDetailsPurchaseDate) && isset($purchaseDetailsQuantity)) {
		...
		if ($originalPurchaseQuantityStatement->rowCount() > 0) {
			...
			if ($originalOrderItemNumber !== $purchaseDetailsItemNumber) {
				...
				$updatePurchaseDetailsSql = 'UPDATE purchase SET itemNumber = :itemNumber, purchaseDate = :purchaseDate, itemName = :itemName, quantity = :quantity, vendorName = :vendorName, vendorID = :vendorID WHERE purchaseID = :purchaseID';
				$updatePurchaseDetailsStatement = $conn->prepare($updatePurchaseDetailsSql);
				$updatePurchaseDetailsStatement->execute(['itemNumber' => $purchaseDetailsItemNumber, 'purchaseDate' => $purchaseDetailsPurchaseDate, 'itemName' => $purchaseDetailsItemName, 'quantity' => $purchaseDetailsQuantity, 'vendorName' => $purchaseDetailsVendorName, 'vendorID' => $vendorID, 'purchaseID' => $purchaseDetailsPurchaseID]);
				...
				if ($stockStatement->rowCount() > 0) {
					...
					$updatePurchaseDetailsSql = 'UPDATE purchase SET purchaseDate = :purchaseDate, quantity = :quantity, vendorName = :vendorName, vendorID = :vendorID WHERE purchaseID = :purchaseID';
					$updatePurchaseDetailsStatement = $conn->prepare($updatePurchaseDetailsSql);
					$updatePurchaseDetailsStatement->execute(['purchaseDate' => $purchaseDetailsPurchaseDate, 'quantity' => $purchaseDetailsQuantity, 'vendorName' => $purchaseDetailsVendorName, 'vendorID' => $vendorID, 'purchaseID' => $purchaseDetailsPurchaseID]);
