...
if (isset($_POST['purchaseDetailsItemNumber'])) {
	...
	if (isset($purchaseDetailsItemNumber) && isset($purchaseDetailsPurchaseDate) && isset($purchaseDetailsItemName) && isset($purchaseDetailsQuantity)) {
		...
		if ($stockStatement->rowCount() > 0) {
			...
			$insertPurchaseSql = 'INSERT INTO purchase(itemNumber, purchaseDate, itemName, quantity, vendorName, vendorID) VALUES(:itemNumber, :purchaseDate, :itemName, :quantity, :vendorName, :vendorID)';
			$insertPurchaseStatement = $conn->prepare($insertPurchaseSql);
			$insertPurchaseStatement->execute(['itemNumber' => $purchaseDetailsItemNumber, 'purchaseDate' => $purchaseDetailsPurchaseDate, 'itemName' => $purchaseDetailsItemName, 'quantity' => $purchaseDetailsQuantity, 'vendorName' => $purchaseDetailsVendorName, 'vendorID' => $vendorID]);
